...
export class CartService {
 ...
 // ✅ just declare the property
cartItems$: Observable<CartItemWithDiscounts[]>;

  constructor(
    private authService: AuthService
  ) {

    const initialCartItems = this.safeGetCartItemsOnly(); // use a safe version

    this.cartItemsSubject = new BehaviorSubject<CartItemWithDiscounts[]>(initialCartItems);
  ...
  getFullCart(): FullCartData | null {
    ...
    try {
      ...
      return parsed as FullCartData;
    } catch (e) {
      console.error("Failed to parse full cart:", e);
      return null;
    }
  }  
  ...
  getCartItemsOnly(): CartItemWithDiscounts[] {
    try {
      ...
      console.error("Error in getCartItemsOnly():", e);
      return [];
    }
  }
  ...
  getVariantQuantity(productId: number, variantId: number): number {
    const fullCart = this.getFullCart();
    if (!fullCart || !Array.isArray(fullCart.cartItems)) return 0;
